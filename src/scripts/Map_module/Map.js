...
export default class Map {
		...
		getPlaceCoord(this.town)
			.then((coord) => {
				...
				const promiseArr = requests.map((request) => {
					return getPlaceData(this.place_LON, this.place_LAT, request).then(
						(data) => {
							this.data.push(data);
						}
					);
    ...
    <div class="iw-container">
      ...
      <div class="iw-content">
        <img class="iw-img" src="${
					place.preview.source
				}" height="150px" width="150px" alt="${place.name}"></img>
        <div class="iw-info">${place.wikipedia_extracts.text}</div>
      </div>
      <div class="iw-contacts">
        <div class="iw-address">Address: ${
					place.address.city || place.address.town
				}, ${place.address.country}, ${place.address.postcode}</div>
	...
	createLegend = () => {
		...
		legendCategories.forEach((obj, index) => {
			...
			createDOMElement(
				...
				'legend',
				[
					createDOMElement('div', 'legend-icon', `${icon}`, null, [
						'data-category',
						`${index}`,
					]),
